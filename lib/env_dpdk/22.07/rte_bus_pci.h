...
#define FOREACH_DRIVER_ON_PCIBUS(p) \
	...
	struct rte_pci_device
	{
		RTE_TAILQ_ENTRY(rte_pci_device)
		next;					  /**< Next probed PCI device. */
...
#define RTE_PCI_DEVICE(vend, dev)          \
	...
	struct rte_pci_driver
	{
		RTE_TAILQ_ENTRY(rte_pci_driver)
		next;							   /**< Next in list. */
...
#define RTE_PCI_DRV_NEED_IOVA_AS_VA 0x0040
	 ...
	 */
	__rte_experimental int rte_pci_set_bus_master(struct rte_pci_device *dev, bool enable);
