...
struct spdk_nvme_poll_group
{
	...
	STAILQ_HEAD(, spdk_nvme_transport_poll_group)
	tgroups;
...
struct spdk_nvme_transport_poll_group
{
	...
	const struct spdk_nvme_transport *transport;
	STAILQ_HEAD(, spdk_nvme_qpair)
	connected_qpairs;
	STAILQ_HEAD(, spdk_nvme_qpair)
	disconnected_qpairs;
	STAILQ_ENTRY(spdk_nvme_transport_poll_group)
	link;
...
struct spdk_nvme_ns
{
	...
	RB_ENTRY(spdk_nvme_ns)
	node;
};

#define CTRLR_STRING(ctrlr) \
	(spdk_nvme_trtype_is_fabrics(ctrlr->trid.trtype) ? ctrlr->trid.subnqn : ctrlr->trid.traddr)
...
#else
#define NVME_CTRLR_DEBUGLOG(ctrlr, ...) \
	do                                  \
	{                                   \
	} while (0)
#endif
...
{
	struct spdk_nvme_cpl cpl;
	STAILQ_ENTRY(spdk_nvme_ctrlr_aer_completion)
	link;
};
...
struct spdk_nvme_ctrlr_process
{
	...
	TAILQ_ENTRY(spdk_nvme_ctrlr_process)
	tailq;
	...
	STAILQ_HEAD(, spdk_nvme_ctrlr_aer_completion)
	async_events;
};
...
struct spdk_nvme_ctrlr
{
	...
	/* Tree of namespaces */
	RB_HEAD(nvme_ns_tree, spdk_nvme_ns)
	ns;
	...
	TAILQ_HEAD(, spdk_nvme_ctrlr_process)
	active_procs;
	...
	STAILQ_HEAD(, nvme_io_msg_producer)
	io_producers;
	...
	STAILQ_HEAD(, nvme_register_completion)
	register_operations;
...
struct nvme_ctrlr_detach_ctx
{
	...
	TAILQ_ENTRY(nvme_ctrlr_detach_ctx)
	link;
};

struct spdk_nvme_detach_ctx
{
	TAILQ_HEAD(, nvme_ctrlr_detach_ctx)
	head;
};
...
int nvme_transport_ctrlr_control_msix_enable(struct spdk_nvme_ctrlr *ctrlr, uint16_t index, bool enable); // 我添加的
...
int32_t nvme_transport_qpair_check_completions(struct spdk_nvme_qpair *qpair,
											   uint32_t max_completions);
...
int64_t nvme_transport_poll_group_check_completions(struct spdk_nvme_transport_poll_group *tgroup,
													uint32_t completions_per_qpair, spdk_nvme_disconnected_qpair_cb disconnected_qpair_cb);
