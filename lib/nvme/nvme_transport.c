...
struct spdk_nvme_transport
{
	...
	TAILQ_ENTRY(spdk_nvme_transport)
	link;
};

TAILQ_HEAD(nvme_transport_list, spdk_nvme_transport)
...
int nvme_transport_alloc_msix(struct spdk_nvme_ctrlr *ctrlr, uint16_t index, uint16_t flag)
{
	...
	{
		return transport->ops.ctrlr_alloc_msix(ctrlr, index, flag);
	}
...
int nvme_transport_ctrlr_control_msix_enable(struct spdk_nvme_ctrlr *ctrlr, uint16_t index, bool enable)
{
	const struct spdk_nvme_transport *transport = nvme_get_transport(ctrlr->trid.trstring);

	assert(transport != NULL);
	if (transport->ops.ctrlr_control_msix_enable)
	{
		return transport->ops.ctrlr_control_msix_enable(ctrlr, index, enable);
	}
...
int nvme_transport_ctrlr_set_reg_4_async(struct spdk_nvme_ctrlr *ctrlr, uint32_t offset, uint32_t value,
...
int nvme_transport_ctrlr_set_reg_8_async(struct spdk_nvme_ctrlr *ctrlr, uint32_t offset, uint64_t value,
										 spdk_nvme_reg_cb cb_fn, void *cb_arg)
...
void nvme_transport_ctrlr_delete_io_qpair(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair)
{
	const struct spdk_nvme_transport *transport = nvme_get_transport(ctrlr->trid.trstring);
	int rc;
...
void nvme_transport_ctrlr_disconnect_qpair(struct spdk_nvme_ctrlr *ctrlr, struct spdk_nvme_qpair *qpair)
{
	const struct spdk_nvme_transport *transport = nvme_get_transport(ctrlr->trid.trstring);
...
void nvme_transport_poll_group_check_disconnected_qpairs(struct spdk_nvme_transport_poll_group *tgroup,
