...
struct ns_entry
{
    ...
    TAILQ_ENTRY(ns_entry)
    link;
    struct spdk_plus_smart_nvme *nvme_dev;
};
...
read_complete(void *arg, const struct spdk_nvme_cpl *completion)
{
    ...
    {
        //  spdk_nvme_qpair_print_completion(sequence->ns_entry->qpair, (struct spdk_nvme_cpl *)completion); TODO: 如何处理
...
write_complete(void *arg, const struct spdk_nvme_cpl *completion)
{
    ...
    if (spdk_nvme_cpl_is_error(completion))
    {
        //  spdk_nvme_qpair_print_completion(sequence->ns_entry->qpair, (struct spdk_nvme_cpl *)completion); TODO: 这个怎么处理
        fprintf(stderr, "I/O error status: %s\n", spdk_nvme_cpl_get_status_string(&completion->status));
    ...
    rc = spdk_plus_nvme_ns_cmd_read(sequence->ns_entry->ns, sequence->ns_entry->nvme_dev,
                                    sequence->buf, 0, /* starting LBA of the zone to reset */
                                    1,                /* number of LBAs to read */
                                    read_complete,
                                    (void *)sequence,
                                    0);
    if (rc != 0)
...
{
    if (spdk_plus_nvme_zns_reset_zone(sequence->ns_entry->ns, sequence->ns_entry->nvme_dev,
    ...
    {
        spdk_plus_nvme_process_completions(sequence->ns_entry->nvme_dev, 0);
    }
...
hello_world(void)
{
    ...
    TAILQ_FOREACH(ns_entry, &g_namespaces, link)
    {
         ...
         *  process.
         */
        rc = spdk_plus_nvme_ns_cmd_write(ns_entry->ns, ns_entry->nvme_dev,
        ...
        while (!sequence.is_completed)
        {
            spdk_plus_nvme_process_completions(ns_entry->nvme_dev, 0);
        }
         ...
         */
        spdk_plus_nvme_ctrlr_free_io_qpair(ns_entry->nvme_dev);
    }
}
         ...
         struct spdk_nvme_ctrlr_opts *opts)
{
    ...
    if (spdk_nvme_transport_id_compare(&g_trid, trid) == 0)
    {
        printf("Found matching controller\n");
        return true;
    }
    return false;
}
...
usage(const char *program_name)
{
    ...
    printf("\t[-f PCIe address of NVMe device]\n");
...
parse_args(int argc, char **argv, struct spdk_env_opts *env_opts)
{
    ...
    while ((op = getopt(argc, argv, "f:d:ghi:r:L:V")) != -1)
    {
        switch (op)
        {
        case 'V':
...
int main(int argc, char **argv)
{
    ...
    opts.name = "smart_nvme_hello_world";
