...
#define SPDK_PCI_DRIVER_REGISTER(name, id_table, flags)                             \
	...
	int spdk_pci_device_control_spec_interrupt(struct spdk_pci_device *dev, uint32_t index, bool enable);
	...
	struct spdk_pci_device_provider
	{
		...
		TAILQ_ENTRY(spdk_pci_device_provider)
		tailq;
	};
